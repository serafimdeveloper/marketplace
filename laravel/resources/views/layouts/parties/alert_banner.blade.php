<div class="alertbox" id="alert-banner">
    <div class="alertbox-container">
        ...
        <div class="alertbox-content">
            ...
            @if(isset($result))
                {!! Form::model($result,['class' => 'form-modern pop-form form-banner', 'route' =>['account.report.banner.update',$result->id], 'method' => 'POST']) !!}
            @else
                {!! Form::open(['class' => 'form-modern pop-form form-banner', 'route' => ['account.report.banner.store'], 'method' => 'POST']) !!}
            @endif
                ...
                <div class="colbox">
                    <div class="colbox-2">
                        <label>
                            <span>Início</span>
                            {!! Form::input('datetime-local', 'date_start',  isset($result->date_start) ? $result->date_start->format('Y-m-d\TH:i:s') : null, ['class' => 'datetimepicker_datetime', 'placehoder' => 'data e hora inicial', 'required' => true]) !!}
                        </label>
                    </div>
                    <div class="colbox-2">
                        <label>
                            <span>Fim</span>
                            {!! Form::input('datetime-local', 'date_end',  isset($result->date_end) ? $result->date_end->format('Y-m-d\TH:i:s') : null, ['class' => 'datetimepicker_datetime', 'placehoder' => 'data e hora final', 'required' => true]) !!}
                        </label>
                    </div>
                </div>
