...
@section('content')
    ...
    <section class="panel-content">
        ...
        @if(isset($seller))
            {!!Form::model($seller,['url' => '/account/seller/update', 'method' => 'POST', 'class' => 'form-modern pop-form', 'enctype'=>'multipart/form-data'])!!}
        @else
            {!! Form::open(['route' => ['account.seller.store'], 'method' => 'POST', 'class' => 'form-modern pop-form', 'enctype'=>'multipart/form-data']) !!}
        @endif
        <div class="colbox">
            <div class="colbox-2">
                <label>
                ...
                <div class="form-modern">
                    ...
                    @if(!isset($seller->photo_document) || !isset($seller->proof_address))
                        <p>
                    ...
                    <div class="txt-center">
                        ...
                        @else
                            <p>
                                <img src="{{url('imagem/vendedor/'.$seller->photo_document.'?w=180&h=120')}}"><br>
                        ...
                        @if(!$isDocs['document'])
                            ...
                            <div class="file" style="border:1px solid #B0BEC5;padding: 10px;">
                                {!! Form::file('proof_address') !!}
                                <input type="text" value="{{ old('proof_address') }}">
                                <button type="button" class="btn btn-orange">imagem</button>
                                <div class="clear-both"></div>
                                <span class="alert{{ $errors->has('proof_address') ? '' : ' hidden' }}">{{ $errors->first('proof_address') }}</span>
                            </div>
                        @else
                            <p>
                                @if(substr($seller->proof_address, -3) == 'pdf')
                                    {{--<a href="{{url('imagem/vendedor/'.$seller->proof_address)}}" target="_blank">ver comprovante</a><br>--}}
                                    <span class="fa fa-file-pdf-o fontem-20"></span><br>
                                @else
                                    <img src="{{url('imagem/vendedor/'.$seller->proof_address.'?w=180&h=120')}}"><br>
                                @endif
        ...
        <div class="txt-center">
            <button type="submit" class="btn btn-popmartin"
                    style="margin-top:15px">{{isset($seller) ? 'atualizar' : 'gravar'}}</button>
        </div>
        {!! Form::close() !!}
    </section>
    <div class="clear-both"></div>
