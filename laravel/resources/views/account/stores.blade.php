...
@section('content')
    ...
    <section class="panel-content">
        ...
        @if(isset($store))
            {!!Form::model($store,['route'=>['account.seller.stores.update'], 'method'=>'POST', 'class' => 'form-modern pop-form', 'enctype'=>'multipart/form-data'])!!}
        @else
            {!! Form::open(['route' => ['account.seller.stores.store'], 'method' => 'POST', 'class' => 'form-modern pop-form', 'enctype'=>'multipart/form-data']) !!}
        @endif
        <div class="colbox">
            <div class="colbox-2">
                <label>
                ...
                <div class="checkbox-container padding10">
                    ...
                    <div class="checkboxies">
                        <label class="radio select_type_sallesman" style="border: none;">
                            @if(isset($store))
                                <span><span class="fa {{ ($store->type_seller === 'F') ? 'fa-check-circle-o c-green':'fa-circle-o'}}"></span> física</span>
                                {!! Form::radio('type_seller','F',($store->type_seller === 'F') ? true : ' ') !!}
                            @else
                                <span><span class="fa fa-check-circle-o c-green"></span> física</span>
                                {!! Form::radio('type_seller','F', true) !!}
                            @endif
                        </label>
                        <label class="radio select_type_sallesman" style="border: none;">
                            @if(isset($store))
                                <span><span class="fa {{ ($store->type_seller === 'J') ? 'fa-check-circle-o c-green':'fa-circle-o'}}"></span> jurídica</span>
                                {!! Form::radio('type_seller','J', ($store->type_seller === 'J') ? true : ' ') !!}
                            @else
                                <span><span class="fa fa-circle-o"></span> jurídica</span>
                                {!! Form::radio('type_seller','J') !!}
                            @endif

                        </label>
                    </div>
                    <span class="alert{{ $errors->has('type_seller') ? '' : ' hidden' }}">{{ $errors->first('type_seller') }}</span>
                </div>
                @if(isset($store))
                    <div class="selects_people select_cpf" style="{{ $store->type_seller === 'F' ? 'display: block' : ''}}">
                        @else
                            ...
                            @if(isset($store))
                                <div class="selects_people select_cnpj" style="{{ $store->type_seller === 'J' ? 'display: block' : ''}}">
                                    @else
                                ...
                                <div class="colbox-2">
                                    <label>
                                        ...
                                        <span class="limiter-result" for="sobre1" data-limit="500">{{ $store ? 500 - strlen($store->about) : '500' }}</span>
                                    </label>
                                    <label>
                                        ...
                                        <span class="limiter-result" for="sobre2" data-limit="500">{{ $store ? 500 - strlen($store->exchange_policy) : '500' }}</span>
                                    </label>
                                    <label>
                                        ...
                                        <span class="limiter-result" for="sobre3" data-limit="500">{{ $store ? 500 - strlen($store->freight_policy) : '500' }}</span>
                                    </label>
                                </div>
                        ...
                        @if(isset($store))
                            @if(isset($address->id))
                                ...
                                <div class="panel-end" id="end_{{$address->id}}">
                                    ...
                                    <a href="javascript:void(0)" class="panel-end-edit vertical-flex jq-address" data-alertbox="alert-address" data-id="{{$address->id}}" data-action="store">editar|excluir</a>
                                </div>
                            @else
                                <div id="isAddress">
