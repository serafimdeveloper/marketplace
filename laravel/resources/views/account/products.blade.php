    ...
    <section class="panel-content">
        <header class="pop-title">
            ...
            <div class="pop-tile-menu">
                <a href="{{route('account.seller.products.create')}}" class="btn btn-smallextreme btn-popmartin"><i
                            class="fa fa-plus vertical-middle"></i> adicionar novo produto</a>
            </div>
        </header>
        @if(!$products->first())
            <p class="trigger notice fontem-14">Não há produto cadastrado <br>
                <a href="{{route('account.seller.products.create')}}" class="btn btn-smallextreme btn-popmartin">Cadastrar meu primeiro produto</a>
            </p>
        @else
            <table id="pop-messages" class="table table-action fontem-12">
                ...
                @forelse ($products as $product)
                    <tr>
                        <td><img src="{{ ($product->galleries->first() ? url('imagem/produto/'.$product->galleries->first()->image.'?w=42&h=42') : url('/imagem/popmartin/img-exemple.jpg?w=42&h=42')) }}"
                                 alt="[]" title=""></td>
                        <td>{{$product->name}}</td>
                        ...
                        <td class="txt-center">
                            <a href="{{route('account.seller.products.edit',$product->id)}}" class="t-btn t-edit">detalhes</a>
            ...
            <br>
            <div class="txt-right">
                <a href="{{route('account.seller.products.create')}}" class="btn btn-small btn-popmartin">
