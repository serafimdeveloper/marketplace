    ...
    <table id="jq-search-table-result" class="table table-action">
        ...
        @forelse($result as $user)
            <tr class="trUser{{ $user->id }}">
                ...
                <td>{{$user->email}}</td>
                <td>{{$user->last_access ? $user->last_access->format('d/m/Y H:i:s') : 'sem acesso'}}</td>
    ...
    <table id="jq-search-table-result" class="table table-action">
        ...
        @forelse($result as $seller)
            <tr {!! !$seller->read ? 'class="t-unread"' : '' !!}>
                ...
                <td>{{isset($seller->store->address) ? $seller->store->address->city.'/'.$seller->store->address->state : '-'}}</td>
