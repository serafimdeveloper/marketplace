    ...
    <section class="panel-content">
        ...
        <div class="padding10-20">
                ...
                <div class="coltable-6">
                    <p>
                        <span class="fontw-500">Status:</span> <span
                                class="fontw-500 c-{{ $request->request_status->trigger }}">{{ $request->request_status->description }}</span><br>
                ...
                @foreach($request->products as $product)
                    <tr>
                        <td class="txt-center" style="max-width: 100px;"><img
                                    src="{{ url('imagem/produto/' . $product->galleries[0]->image.'?w=60&h=60&fit=crop') }}">
                        </td>
                        <td>
            ...
            <table class="table table-action">
                ...
                @if($address['receiver'])
                    <tr>
                        <td>{{ $request->type_freight ? $request->type_freight->name : 'sem frete' }}</td>
                        <td>
                            <span>{{ $address['receiver']->name }}</span><br>
                            <span>{{ $address['receiver']->zip_code }} ({{ $address['receiver']->state }})</span><br>
                        </td>
                        <td class="bold"><span class="fontem-12">R${{ number_format($request->freight_price, 2, ',', '') }}</span>
                        </td>
                    </tr>
                @endif
                </tbody>
            ...
            @endif
            <hr>
            <p class="fontem-22 fontw-500">Total do pedido
                <span class="fl-right c-pop fontw-900">
                    R${{number_format(amount_products_final($request->products,$request->freight_price) + $request->amount_interest,2,',','.')}}
                </span>
            </p>
