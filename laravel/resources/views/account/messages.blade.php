...
@section('content')
    ...
    <section class="panel-content">
        ...
        <br>
        <div>
            <a href="{{route('account.messages.box',['type'=> $type, 'box' => 'received'])}}"
               class="btn btn-small {{ $box === 'received' ? 'btn-popmartin-trans' : 'btn-popmartin'}}">
                {!! ($box === 'received' ? '<i class="fa fa-angle-down"></i>' : '') !!}
                Caixa de entrada

            </a>
            <a href="{{route('account.messages.box',['type'=> $type, 'box' => 'send'])}}"
            ...
            <table id="pop-messages" class="table table-action">
                ...
                @foreach($messages as $msg)
                    ...
                    @endif
                            ...
                            <td>
                                <a href="{{ route('account.message.info', ['type' => $box, 'id' => ($box === 'received') ? $msg->first()->id : $msg->id]) }}">{!!  substr(($box === 'received') ? $msg->first()->content : $msg->content, 0, 60) !!}
                                    ...</a></td>
