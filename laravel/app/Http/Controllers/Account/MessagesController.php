...
class MessagesController extends AbstractController {
    ...
    public function index($type, $box = 'received'){
        ...
        if($type == 'user'){
            ...
            return view('account.messages', compact('messages', 'type', 'box'));
        }elseif($type == 'store'){
    ...
    public function show($box, $id){
        ...
        if($message = $this->repo->get($id)){
            if(Gate::allows('read_message', [$message, $box])){
                ...
                $eu = $user->name;
                return view('account.message_info', compact('messages', 'message', 'box', 'eu'));
            }
        }
    ...
    public function comments(Request $request, $type, $id){
        ...
        if($type === "request"){
            if($req = $this->req->find($id)){
                ...
                $store = $req->store;
                if(preg_match("/account\/seller\/sale/", redirect()->back()->getTargetUrl())){
        ...
        if($message = $this->repo->store($dados)){
            $data = ['email' => $store->seller->user->email, 'name' => $store->name, 'id' => $message->id, 'message_type' => 'received'];
            if(preg_match("/account\/seller\/sale/", redirect()->back()->getTargetUrl())){
        ...
        if($model = $this->repo->get($id, $this->columns, $this->with)){
            ...
            if($message = $this->repo->store($dados)){
                ...
                if(($model->sender instanceof Store && $box === 'received') || ($model->recipient instanceof Store && $box === 'send')){
                    $data = ['email' => $recipient->seller->user->email, 'name' => $recipient->name, 'id' => $message->id, 'message_type' => 'received'];
    ...
    private function read_type($user, $message, $messages, $box){
        if($box === 'received'){
            ...
            if($recipient instanceof User){
                ...
                if($message->recipient_id === $user->seller->store->id){
                    $messages->where('recipient_id', $user->seller->store->id)->where('message_id',$message->message_id)
                        ->where('status','received')->each(function($message){
                            $message->update(['status' => 'readed']);
                        });
                }
