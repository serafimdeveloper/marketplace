...
abstract class BaseRepository
{
    ...
    public function filter($collections, $select){
        $retorno = [];
        foreach($select as $campo){
            if(isset($collections[$campo])){
                if(is_array($collections[$campo])){
                    $this->filter($collections[$campo],$select);
                }else{
                    $retorno[$campo] = $collections[$campo];
                }
            }
        }
        return $retorno;
